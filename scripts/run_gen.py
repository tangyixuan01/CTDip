...
class Test(object):
    ...
    def __init__(self, stat, seed="", proc_num=-1, blame=False, creduce_makefile=None, lock=None):
        ...
        is_mutate_success = True
        mutation_operator_label = ''
        self.mutation_operator_label = mutation_operator_label
        ...
        if self.is_time_expired:
            ...
            stat.update_yarpgen_runs(runfail)
            self.stat.seed_failed(self.seed)
        # ^ if mutate failed, we should also update seed_failed
        elif not is_mutate_success:
...
def prepare_env_and_start_testing(out_dir, timeout, targets, num_jobs, config_file, seeds_option_value, blame, creduce,
    ...
    # TODO get seeds from file
    # print(os.path.join(os.path.abspath(os.path.dirname(__file__)), 'seeds', 'seeds_1t_0.txt'))
    seeds_option_value = ""
    with open(os.path.join(os.path.join(os.path.abspath(os.path.dirname(__file__))),
                                        'seeds', 
                                        'seeds4exp_10t_new.txt'), # ! 修改这个文件名来指定种子文件
                           'r') as f:
        seeds = f.readlines()
        seeds = [s.strip() for s in seeds]
        # print('>> seeds length:', len(seeds))
        for s in seeds:
            seeds_option_value += "S_" + s + "/,"
    seeds_option_value = seeds_option_value[:-1]
    # print('>> seeds option value:', seeds_option_value)

    # TODO random generate seeds
    # import random
    # for i in range(3000):
    #     seeds_option_value += "S_" + str(random.randint(1, 999999999)) + "/,"
    # seeds_option_value = seeds_option_value[:-1]

    # for i in range(1000001, 1010001):
    #     seeds_option_value += "S_" + str(i) + "/,"
    # seeds_option_value = seeds_option_value[:-1]
