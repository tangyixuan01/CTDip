...
grammarinator_fuzzer_dir = "/media/jwzeng/zjw_SSD/workplace/experiment/generator/grammarinator/examples-20.7/fuzzer"
...
yarpgen_timeout = 60        # original value is 60
compiler_timeout = 60       # original value is 1200
run_timeout = 60            # original value is 300
stat_update_delay = 10      # original value is 10
tmp_cleanup_delay = 3600    # original value is 3600
creduce_timeout = 3600 * 72 # original value is 3600 * 72
...
class Test(object):
    ...
    # ^ new status
    STATUS_mutate_fail=7

    # Static variables
    # Don't save anything other than log-file if compile time expires
    ignore_comp_time_exp = True

    # Default is not use sanitizer to check test program's ub
    is_need_sanitizer = False

    # Fuzzer name, default is file
    fuzzer_name = "file"

    # Fuzzer nums, default is -1
    fuzzer_nums = 0
    ...
    def __init__(self, stat, seed="", proc_num=-1, blame=False, creduce_makefile=None, lock=None):
        # Run yarpgen generator
        # yarpgen_run_list = [".." + os.sep + "yarpgen",
        #                     "--std=" + common.StdID.get_pretty_std_name(common.selected_standard)]
        # yarpgen_run_list += ["-s", seed] if seed else []
        ...
        # Run csmith generator
        # csmith_run_list = ['csmith', '-o', 'test.c']
        # csmith_run_list += ["-s", seed] if seed else []
        # self.yarpgen_cmd = " ".join(str(p) for p in csmith_run_list)
        # self.ret_code, self.stdout, self.stderr, self.is_time_expired, self.elapsed_time = \
        #     common.run_cmd(csmith_run_list, yarpgen_timeout, proc_num, yarpgen_mem_limit)

        self.ret_code = 0
        self.is_time_expired = False
        if Test.fuzzer_name == "file":
            # Copy seed to the current directory
            if os.path.exists(seed):
                shutil.copyfile(seed, 'test.c')
            else:
                common.log_msg(logging.ERROR, "Seed file " + seed + " does not exist", True)
                raise
        elif Test.fuzzer_name == "grammarinator":
            gen_grammarinator_program = ['grammarinator-generate', 'CGenerator.CGenerator', 
                                         '-d', '20', '-o', 'test.c', '-n', '1', '--sys-path',
                                         grammarinator_fuzzer_dir]
            self.ret_code, self.stdout, self.stderr, self.is_time_expired, self.elapsed_time =\
                common.run_cmd(gen_grammarinator_program, yarpgen_timeout, proc_num, yarpgen_mem_limit)
        ...
        self.lock = lock
        # # ! this is for yarpgen
        # # is_mutate_success, mutation_operator_label = mutate_program(test_dir=os.getcwd(), proc_num=proc_num, seed=self.seed, lock=lock)
        # # ! this is for csmith
        ...
        self.mutation_operator_label = ''

        is_free_ub = True
        if Test.is_need_sanitizer:
            is_free_ub = check_program_by_all_sanitizers(
                SANITIZER, ['test.c'], 
                os.getcwd(), proc_num=proc_num, 
                seed=seed, lock=lock,
                mutation_operator_label=self.mutation_operator_label)

        # Update statistics and set the status
        if not is_free_ub:
            common.log_msg(logging.WARNING, "Mutate has failed (" + mutate_fail + ")")
            self.status = self.STATUS_mutate_fail
            stat.update_yarpgen_runs(mutate_fail)
    ...
    def status_string(self):
        ...
        # ^ new status
        elif self.status == self.STATUS_mutate_fail:         return "mutate_fail"
        else: raise

    # Save test
    def save(self, lock):
        if self.status != self.STATUS_fail_timeout and \
              self.status != self.STATUS_fail and \
                self.status != self.STATUS_mutate_fail:
            raise
    ...
    def handle_results(self, lock):
        ...
        # Handle miscompares.
        # common.log_msg(logging.INFO, '---------222')
    ...
    def verify_results(self, lock):
        ...
        # common.log_msg(logging.INFO, '---------333')
        # Check if test passed.
        if len(results) == 1 and not "ERROR" in next(iter(results)):
            # common.log_msg(logging.INFO, '---------444')
            return
        elif len(results) == 2:
            # common.log_msg(logging.INFO, '---------555')
        ...
        if self.creduce and good_runs:
            # common.log_msg(logging.INFO, '---------888')
    ...
    def build_log(self, bad_runs=[], good_runs=[]):
        ...
        log.write("Mutation operator label: " + self.mutation_operator_label + "\n")
        log.write("\n\n")
    ...
    def do_creduce_miscompare(self, good_runs, bad_runs):
        # common.log_msg(logging.INFO, '---------999')
...
class TestRun(object):
    ...
    def save(self, lock):
        ...
        save_test(lock,
                   ...
                   test_name=self.test.seed.split('/')[-1][:-2])
    ...
    def build_log(self):
        ...
        if self.test.blame:
            ...
            log.write("Optimization to blame: " + self.blame_phase + "\n")
        # ^ Mutation operator label
        log.write("Mutation operator label: " + self.test.mutation_operator_label + "\n")
...
def do_blame(test_obj, test_programs, good_result, target_to_blame):
...
def form_statistics(stat, targets, prev_len, tasks=None):
    ...
    # ^ mutate fail
    verbose_stat_str += "\t" + mutate_fail + " : " + str(stat.get_yarpgen_runs(mutate_fail)) + "\n"
    ...
    # ^ mutate fail total
    total_mutate_fail = stat.get_yarpgen_runs(mutate_fail)
    total_runs = 0
    total_ok = 0
    ...
    stat_str += "Errors(g/ct/c/rt/r/d/m): " + str(total_gen_errors) + "/"
    ...
    stat_str += str(total_mutate_fail)
...
def print_compilers_version(targets, out_dir):
    for i in targets:
        ...
        with open(os.path.join(out_dir, 'compiler_version.log'), 'a+') as f:
            compiler = gen_test_makefile.CompilerSpecs.all_comp_specs[i].name + ': ' + comp_exec_name
            if len(compiler) <= 80:
                f.write(compiler + ' ' + ('=' * (80 - len(compiler) - 1)) + '\n')
            else:
                f.write(compiler + ' ===\n')
...
def get_all_c_files_from_dir(dir):
    c_files = []
...
def prepare_env_and_start_testing(out_dir, timeout, targets, num_jobs, config_file, programs_dir, blame, creduce,
    ...
    test_programs = []
    if Test.fuzzer_name == 'file':
        print('>> programs_dir:', programs_dir)
        if programs_dir is not None and os.path.isdir(programs_dir):
            test_programs = get_all_c_files_from_dir(programs_dir)
            if len(test_programs) == 0:
                print("Can't find any .c files in programs directory. Please, specify it in config file.")
                exit(1)
        else:
            print("Can't find programs directory. Please, specify it in config file.")
            exit(1)
    elif Test.fuzzer_name == 'grammarinator':
        test_programs = list(range(Test.fuzzer_nums))
        test_programs = [str(x) for x in test_programs]
        print(test_programs)

    # print(test_programs)
    common.log_msg(logging.INFO, 'The fuzzer name is: ' + Test.fuzzer_name, forced_duplication=True)
    common.log_msg(logging.INFO, 'The fuzzer nums is: ' + str(Test.fuzzer_nums), forced_duplication=True)
    common.log_msg(logging.INFO, 'All test files: ' + str(test_programs), forced_duplication=True)

    task_queue = None
    if test_programs:
        # seeds = proccess_seeds(test_programs)
        task_queue = multiprocessing.Queue()
        for s in test_programs:
            task_queue.put(s)
        if len(test_programs) < num_jobs:
            num_jobs = len(test_programs)

    print_compilers_version(targets, out_dir)
    ...
    # get the current time, foramt: 2016-12-30 15-30-01
    now = datetime.datetime.now()
    now_time = now.strftime('%Y-%m-%d_%H-%M-%S')
    print('>> now', now_time)

    # get time consume
    time_consume = strfdelta(now - script_start_time, "{days} d {hours}:{minutes}:{seconds}")
    print('>> time consume:', time_consume)

    # save as statistics_[date].txt
    with open(os.path.join(os.getcwd(), 'statistics_' + str(now_time) + '.log'), 'w') as f:
        f.write(verbose_stat_str)
        f.write("\n")
        f.write("start time: ")
        f.write(script_start_time.strftime('%Y-%m-%d %H-%M-%S'))
        f.write("\n")
        f.write("end time: ")
        f.write(now_time)
        f.write("\n")
        f.write("time consume: ")
        f.write(time_consume)
...
def gen_and_test(num, makefile, lock, end_time, task_queue, stat, targets, blame, creduce_makefile, stat_targets):
    ...
    while inf or end_time > time.time():
        ...
        test = Test(stat=stat, seed=seed, proc_num=num, blame=blame,
                    creduce_makefile=creduce_makefile, lock=lock)
        if not test.is_ok():
            test.save(lock)
            continue
        ...
        # common.log_msg(logging.INFO, '---------111')
...
Use specified folder for testing
    ...
    parser.add_argument("--target", dest="target", default="clang ubsan_clang gcc ubsan_gcc", type=str,
    ...
    parser.add_argument("--programs-dir", dest="programs_dir", default='', type=str,
                        help="Directory with programs to test.")
    parser.add_argument("--is-need-sanitizer", dest="is_need_sanitizer", default=False, action="store_true",
                        help="Don't use sanitizer when testing. (Default is not use sanitizer to check test program's ub.)")
    parser.add_argument("--fuzzer-name", dest="fuzzer_name", default="file", type=str,
                        help="Fuzzer name. (Default is file.) Other choice: Grammarinator")
    parser.add_argument("--fuzzer-nums", dest="fuzzer_nums", default="0", type=str,
                        help="模糊测试的数量，如果为0，直接结束")
    ...
    Test.is_need_sanitizer = args.is_need_sanitizer
    Test.fuzzer_name = args.fuzzer_name
    Test.fuzzer_nums = int(args.fuzzer_nums)

    prepare_env_and_start_testing(os.path.abspath(args.out_dir), args.timeout, targets, args.num_jobs,
                                  args.config_file, args.programs_dir, args.blame, args.creduce,
